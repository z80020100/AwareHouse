var order_info=new Object();order_info["share_array"]=[];order_info["table_num"]="16";order_info["people_num"]="2";order_info["share_array"][0]=new Object();order_info["share_array"][0]["items_array"]=[];var items_array_length=0;$(document).ready(function(){$("button[id^='m']").css({"background-color":"#ffc952", "color":"#ff7473"}); $("#nav_cross").click(function(){$(".w3-sidenav").toggle(); $("#main").css({"marginLeft":"0%"}); $(".w3-sidenav").css({"display":"none"}); $(".w3-opennav").css({"display":"inline-block"});}); $("#nav_open").click(function(){$(".w3-sidenav").toggle(); $("#main").css({"marginLeft":"20%"}); $(".w3-sidenav").css({"display":"block", "width":"20%"}); $(".w3-opennav").css({"display":"none"});}); $("#nav_open").css({"position":"fixed"}); $("#open_cart").css({"position":"fixed", "right":"7%", "bottom":"7%", "z-index":"2"}); $("#open_cart2").css({"position":"fixed", "right":"15%", "bottom":"7%", "z-index":"2"}); $("#open_cart3").css({"position":"fixed", "right":"23%", "bottom":"7%", "z-index":"2"}); $("#open_cart4").css({"position":"fixed", "right":"31%", "bottom":"7%", "z-index":"2"}); $("#nav_cross").click(); $("#open_cart").click(function(){$("#cart_list").css({"display":"block"}); $("button[id^='m']").attr('disabled', 'disabled');}); $("#close_cart").click(function(){$("#cart_list").css({"display":"none"}); $("button[id^='m']").removeAttr('disabled');}); $("button[id^='m']").click(function(){var at_id_array=[]; at_id_array[0]=$(this).attr("data-at_id"); for(var i=0 ; i< $(this).data("ro_at_id").length-1; ++i){at_id_array[i+1]=$(this).data("ro_at_id")[i];}load_ajax(at_id_array); $("#item_name").html($(this)[0].innerHTML); $("#item_price").val(parseInt($(this)[0].value) ); $("#m_price").val(parseInt($(this)[0].value)); $("#m_id").val(($(this)[0].id).slice(1));}); $("#close_confirm").click(function(){$("button[id^='m']").removeAttr('disabled'); $("#amountButton").val("1"); $("#materialblock").empty(); $("#addition_info").val(""); $("#confirm_item").css({"display":"none"});}); $("#add_cart").click(function(){add_row($("#item_name").html(),$("#amountButton").val(), $("#item_price").val()); $("#add_success").css({"position":"fixed", "width":"100%", "height":"0px"}); $("#add_success").css("visibility","visible"); $("#add_success").animate({height:"5%"}); var interval_id=setInterval(function (){$("#add_success").css("visibility","hidden"); $("#add_success").css({"position":"fixed", "width":"100%", "height":"0px"}); clearInterval(interval_id);},2000); $("button[id^='m']").removeAttr('disabled'); $("#close_confirm").click();}); $("#send_order").click(function(){if($("#orderList").find("tr").length <=1){alertify.success("你尚未點選任何品項"); return;}if($("#table_num").val()==""){alertify.success("你尚未填寫桌號"); return;}if($("#people_num").val()==""){alertify.success("你尚未填寫人數"); return;}$("#orderList").find("tr").each(function(index, value){if(index>=1){order_info["share_array"][0]["items_array"].push($(this).data("item_array"));}}); send_total_order();});});
