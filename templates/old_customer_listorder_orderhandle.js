...
var t = order_info.o_time.split(/[- :]/);
var order_starttime = new Date(t[0], t[1]-1, t[2], t[3], t[4], t[5]);

if( order_info.o_estimate_time == 'NULL' ){
	//$("#debug_text").val(  $("#debug_text").val( )+  'yoo1\n');
	var order_disptime = '---';
}
else{
	//$("#debug_text").val(  $("#debug_text").val( )+  'yoo2\n');
	...
	if(timeDiff > 0)
		var order_disptime = diffMins + '分鐘';//order_info.o_estimate_time;  
	else
		var order_disptime = '超過' + diffMins + '分鐘';//order_info.o_estimate_time;  
...
$("#order_list_header").after( orderblock_start + orderSummary_block(order_info.summary_array) + orderShare_block(order_info.share_array)+'</tbody>');
	...
	//將點單時間寫入，未來可能用上
	$('.order_view[order_id|='+order_info.o_id+'] > .order_title > .order_estimate_time').html( order_disptime ); 		//顯示時間
	$('#order_detail_'+order_info.o_id).data("status",  order_info.status);						//將資料庫的status寫入order中
	$('#order_detail_'+order_info.o_id).hide();						//將資料庫的status寫入order中  // customer_listorder
